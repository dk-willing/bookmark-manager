...
export function renderTags(tagsArr, tagsContainer, dataName) {
  tagsArr.forEach((el) => {
    ...
    const stored = JSON.parse(localStorage.getItem(dataName)) || [];
    stored.forEach((obj) => {
...
export function displayCard(
  cardType,
  bookmarkItemsFetched,
  wrapper = mainContentCards
) {
  if (bookmarkItemsFetched.length === 0) {
    ...
    bookmarkItemsFetched.forEach((obj) => {
      ...
      if (cardType === "mainCard") {
        ...
        archieveLi.textContent = `Archive`;
        ...
        const archieveLi = document.createElement("li");
        archieveLi.textContent = `Remove Archive`;
        archieveLi.id = `add-archive`;
        archieveLi.style.fontSize = "1.2rem";
        ul.appendChild(archieveLi);
      }
